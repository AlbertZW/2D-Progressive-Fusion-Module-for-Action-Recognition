...
class TemporalFusionNet(nn.Module):
	def __init__(self, num_class, num_segments,
		...
		self.BottleNeck3 = self.backbone_model.layer3
		...
		self.BottleNeck4 = self.backbone_model.layer4
	...
	def forward(self, x):
		...
		for i in range(num_segments):
			if i == 0:
				...
				x_fused_3 = self.fusion_conv4_3(self.fusion_conv3_3(x[:, i: i + 4, :, :]))
			else:
				if i < 1:
					x_fused_1 = torch.cat((x_fused_1, self.fusion_conv4_1(self.fusion_conv3_1(x[:, i: i + 2, :, :]))), dim=1)
					x_fused_2 = torch.cat((x_fused_2, self.fusion_conv4_2(self.fusion_conv3_2(x[:, i: i + 3, :, :]))), dim=1)
					x_fused_3 = torch.cat((x_fused_3, self.fusion_conv4_3(self.fusion_conv3_3(x[:, i: i + 4, :, :]))), dim=1)
				elif i < 2:
					x_fused_1 = torch.cat((x_fused_1, self.fusion_conv4_1(self.fusion_conv3_1(x[:, i: i + 2, :, :]))), dim=1)
					x_fused_2 = torch.cat((x_fused_2, self.fusion_conv4_2(self.fusion_conv3_2(x[:, i: i + 3, :, :]))), dim=1)
				else:
		...
		x = (x_fused_1 + x_fused_2 + 2 * x_fused_3) / 4
		# (B * C) * 1 * H * W
		#####################################################################################
